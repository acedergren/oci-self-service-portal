 ...
 *
 * Follows the patterns established in session-repository.ts:
...
export interface CreateRunInput {
	/** Workflow definition ID (canonical field name) */
	definitionId?: string;
	/** Alias for definitionId (REST API convention) */
	workflowId?: string;
...
export const workflowRepository = {
	...
	async list(orgIdOrOptions?: string | ListWorkflowsOptions): Promise<WorkflowDefinition[]> {
		// Support string arg as shorthand for { orgId }
	...
	/** Convenience: create a workflow run (delegates to workflowRunRepository). */
	async createRun(definitionId: string, userId?: string): Promise<WorkflowRun> {
		return workflowRunRepository.create({
			definitionId,
			workflowVersion: 1,
			userId
		});
	...
	/** Convenience: update a workflow step (delegates to workflowRunStepRepository). */
	async updateStep(stepId: string, input: UpdateStepInput): Promise<WorkflowStep | null> {
		return workflowRunStepRepository.updateStatus(stepId, input);
	}
};
...
export const workflowRunStepRepository = {
	...
	async updateStatus(id: string, input: UpdateStepInput): Promise<WorkflowStep | null> {
		return withConnection(async (conn) => {
			...
			await conn.execute(`UPDATE workflow_run_steps SET ${sets.join(', ')} WHERE id = :id`, binds);
