 ...
 */
export function recordApproval(toolCallId: string, toolName: string): void {
 ...
 */
export function consumeApproval(toolCallId: string, toolName: string): boolean {
  const entry = approvedToolCalls.get(toolCallId);
  if (!entry) return false;

  // Verify the approval is for the correct tool and hasn't expired
