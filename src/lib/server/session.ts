 ...
 * Tries Oracle DB first; falls back to SQLite if unavailable.
 */
export async function switchToSession(cookies: Cookies, sessionId: string, userId?: string): Promise<boolean> {
  try {
    ...
    return switchToSessionFallback(cookies, sessionId);
  }
}
 ...
 */
function switchToSessionFallback(cookies: Cookies, sessionId: string): boolean {
  const repository = getRepository();
