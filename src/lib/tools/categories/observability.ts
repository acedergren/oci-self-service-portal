...
export const observabilityTools: ToolEntry[] = [
  {
    ...
    execute: (args) => {
      ...
      return slimOCIResponse(executeOCI(cliArgs), [
        'display-name', 'id', 'severity', 'lifecycle-state', 'metric-compartment-id',
        'namespace', 'query', 'is-enabled', 'time-created',
      ]);
    },
  },
  {
    ...
    execute: (args) => {
      ...
      const cliArgs = [
        ...
        '--query-text', args.query as string,
  ...
  },
  {
    ...
    executeAsync: async (args) => {
      ...
      return {
        metricName, period, aggregation, query: mqlQuery,
        namespace: 'oci_computeagent',
        timeRange: { start: startTime.toISOString(), end: now.toISOString(), interval: config.interval },
        data: result,
      };
    },
  },
  {
    ...
    execute: (args) => {
      ...
      return executeOCI([
        ...
        '--compartment-id', compartmentId,
        '--group-by', JSON.stringify(['namespace']),
      ]);
    },
  },
];
