...
export const pricingTools: ToolEntry[] = [
  {
    ...
    parameters: z.object({
      ...
      architecture: z.enum(['x86', 'arm', 'any']).optional().describe('CPU architecture preference'),
      ...
      hoursPerMonth: z.number().optional().default(730).describe('Hours per month (730 for always-on)'),
    ...
    executeAsync: async (args) => {
      ...
      const requirements: WorkloadRequirements = {
        ...
        storage: args.storageGB
          ? { sizeGB: args.storageGB as number, type: args.storageType as 'ssd' | 'hdd' | 'object' | 'archive' | undefined }
          : undefined,
  ...
  },
  {
    ...
    approvalLevel: 'auto',
    parameters: z.object({
      shapeName: z.string().optional().describe('Specific shape name (e.g., VM.Standard.E5.Flex, VM.Standard.A1.Flex)'),
      architecture: z.enum(['x86', 'arm', 'gpu']).optional().describe('Filter by architecture type'),
    ...
    executeAsync: async (args) => {
      ...
      if (args.architecture) {
        const shapes = await client.listComputeShapes({ architecture: args.architecture as 'x86' | 'arm' | 'gpu' });
  ...
  },
  {
    ...
    approvalLevel: 'auto',
    parameters: z.object({
      skuName: z.string().optional().describe('Specific Azure SKU (e.g., Standard_D2s_v3, Standard_B2s)'),
      region: z.string().optional().default('westeurope').describe('Azure region (e.g., westeurope, eastus)'),
    ...
    executeAsync: async (args) => {
      ...
      if (args.skuName) {
        ...
        const monthlyCost = await client.calculateMonthlyCost({
          skuName: args.skuName as string, region, hoursPerMonth: 730,
        });
  ...
  },
  {
    ...
    description: 'Estimate monthly cost for a single cloud provider (OCI, Azure, or AWS). Present the breakdown: Compute | Storage | Networking | Total. Bold the monthly total. For deeper analysis, suggest compareCloudCosts to see all three providers side-by-side.',
    category: 'pricing',
    approvalLevel: 'auto',
    parameters: z.object({
  ...
  },
  {
    ...
    approvalLevel: 'auto',
    parameters: z.object({
      instanceType: z.string().optional().describe('Specific EC2 instance type (e.g., t3.micro, m5.large, c6g.large)'),
      region: z.string().optional().default('eu-west-1').describe('AWS region (e.g., us-east-1, eu-west-1, eu-central-1)'),
    ...
    executeAsync: async (args) => {
      ...
      if (args.instanceType) {
        ...
        const monthlyCost = await client.calculateMonthlyCost({
          instanceType: args.instanceType as string, region, hoursPerMonth: 730,
        });
        return { pricing, monthlyCost, region };
      }
      if (args.architecture) {
        const instances = await client.listEC2Instances({ architecture: args.architecture as 'x86' | 'arm' | 'gpu' });
