...
export const computeTools: ToolEntry[] = [
  {
    ...
    execute: (args) => {
      ...
      return slimOCIResponse(executeOCI(cliArgs), [
        'display-name', 'id', 'lifecycle-state', 'shape', 'shape-config',
        'availability-domain', 'time-created', 'region', 'fault-domain',
      ]);
    },
  },
  {
  ...
  },
  {
    ...
    execute: (args) => {
      return executeOCI([
        ...
        '--compartment-id', args.compartmentId as string,
  ...
  },
  {
    ...
    parameters: z.object({
      compartmentId: compartmentIdSchema,
      operatingSystem: z.string().optional().describe('Filter by OS (e.g., "Oracle Linux", "Canonical Ubuntu")'),
    ...
    execute: (args) => {
      ...
      if (args.operatingSystemVersion) cliArgs.push('--operating-system-version', args.operatingSystemVersion as string);
      if (args.shape) cliArgs.push('--shape', args.shape as string);
      return slimOCIResponse(executeOCI(cliArgs), [
        'display-name', 'id', 'operating-system', 'operating-system-version',
        'time-created', 'size-in-mbs', 'compartment-id',
      ]);
    },
  },
  {
    ...
    execute: (args) => {
      ...
      if (args.availabilityDomain) cliArgs.push('--availability-domain', args.availabilityDomain as string);
      return slimOCIResponse(executeOCI(cliArgs), [
        'shape', 'billing-type', 'processor-description', 'ocpus', 'memory-in-gbs',
        'networking-bandwidth-in-gbps', 'max-vnic-attachments', 'gpu-description',
        'ocpu-options', 'memory-options',
      ]);
    },
  },
  {
  ...
  },
  {
    ...
    executeAsync: async (args) => {
      ...
      const result = await executeOCIAsync([
        ...
        '--from-json', commandDetails,
      ]);

      return {
        instanceId, command, timeoutSeconds, data: result,
  ...
  },
  {
    ...
    execute: (args) => {
      return executeOCI([
        ...
        '--instance-agent-command-id', args.commandId as string,
        '--instance-id', args.instanceId as string,
      ]);
    },
  },
  {
    ...
    execute: (args) => {
      ...
      return executeOCI([
        ...
        '--instanceagent-id', args.instanceId as string,
        '--compartment-id', compartmentId,
      ]);
    },
  },
