...
export const terraformTools: ToolEntry[] = [
  {
    ...
    parameters: z.object({
      ...
      shape: z.string().optional().default('VM.Standard.E4.Flex').describe('Compute shape (e.g., VM.Standard.E4.Flex, VM.Standard.A1.Flex)'),
      ...
      useVariables: z.boolean().optional().default(true).describe('Generate with variables.tf (recommended)'),
    }),
    executeAsync: async (args) => {
      ...
      if (type === 'web-server') {
        const output = generateWebServerTerraform({ name, shape, ocpus, memoryGBs, region, vcnCidr });
        return {
          type: 'web-server',
          files: {
            'main.tf': output.main,
      ...
      const output = generateTerraformCode({
        ...
        subnets: [
          { displayName: `${name}-public`, cidrBlock: vcnCidr.replace('/16', '/24'), isPublic: true, dnsLabel: 'public' },
          { displayName: `${name}-private`, cidrBlock: vcnCidr.replace('.0.0/16', '.1.0/24'), isPublic: false, dnsLabel: 'private' },
        ],
      });

      return {
        type: 'vcn',
        files: { 'main.tf': output.main, 'variables.tf': output.variables, 'outputs.tf': output.outputs },
