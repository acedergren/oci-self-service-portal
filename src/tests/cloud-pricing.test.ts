...
describe('CloudPricingService', () => {
	...
	describe('compareCloudCosts', () => {
		...
		it('includes storage costs when storage requirements provided', async () => {
			...
			expect(comparison.estimates.azure?.breakdown.some((b) => b.category === 'storage')).toBe(
				true
			);
		});
		...
		it('factors in free tier for OCI', async () => {
			...
			expect(
				comparison.estimates.oci?.notes.some(
					(n) => n.toLowerCase().includes('free tier') || n.toLowerCase().includes('always free')
				)
			).toBe(true);
		});
	...
	describe('getRecommendation', () => {
		...
		it('respects provider preference constraint', async () => {
			...
			expect(
				recommendation.reasoning.some(
					(r) => r.toLowerCase().includes('preferred') || r.toLowerCase().includes('preference')
				)
			).toBe(true);
		});
		...
		it('respects budget constraint', async () => {
			...
			expect(
				recommendation.reasoning.some(
					(r) => r.toLowerCase().includes('budget') || r.toLowerCase().includes('exceed')
				)
			).toBe(true);
		});
	});
