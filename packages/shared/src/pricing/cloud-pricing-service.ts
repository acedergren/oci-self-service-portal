...
export class CloudPricingService {
	...
	formatAsMarkdown(comparison: CloudComparison): string {
		...
		if (requirements.storage) {
			lines.push(
				`- **Storage**: ${requirements.storage.sizeGB} GB ${requirements.storage.type ?? 'SSD'}`
			);
		}
	...
	private async estimateAWSCost(requirements: WorkloadRequirements): Promise<CostEstimate> {
		...
		// Networking/egress costs
		if (requirements.networking?.egressGBPerMonth) {
			const billableEgress = Math.max(
				0,
				requirements.networking.egressGBPerMonth - AWS_FREE_EGRESS_GB
			);
		...
		notes.push(
			'AWS Free Tier: t2.micro/t3.micro 750 hrs/month for 12 months only (not always free)'
		);
	...
	private generateRecommendation(
		...
		if (requirements.constraints?.preferredProvider) {
			...
			const preferredCost =
				preferred === 'oci' ? ociEstimate.monthlyTotal : azureEstimate.monthlyTotal;
			...
			if (preferredCost > otherCost) {
				...
				tradeoffs.push(
					`${preferred.toUpperCase()} is ${pctHigher.toFixed(1)}% more expensive for this workload`
				);
			}
