...
<div class="flex flex-col items-center justify-center gap-2">
  <div
    data-testid="spinner"
    class="{sizeClasses[size]} spinner-{size} animate-spin"
  >
...
<div class="flex flex-wrap gap-2 mb-6">
  {#each [
    { label: 'List my instances', cmd: 'List my running compute instances' },
    { label: 'Check databases', cmd: 'Show me all my databases' },
    { label: 'View compartments', cmd: 'List all compartments' },
    { label: 'Network overview', cmd: 'Show network configuration' }
  ] as action}
    <button
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        messages = [...messages, { role: 'assistant', content: data.response, executing: data.executing }];
