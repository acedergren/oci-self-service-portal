...
/**
 * Build Mastra-compatible tools from the registry.
 * Wraps each ToolEntry with Mastra's createTool().
 */
export function buildMastraTools(): Record<string, ReturnType<typeof createTool>> {
	...
	for (const def of toolDefinitions.values()) {
		...
		tools[def.name] = createTool({
			...
			execute: async (input) => {
				if (asyncExecutor) {
					return asyncExecutor(input as Record<string, unknown>);
				}
				if (syncExecutor) {
					return syncExecutor(input as Record<string, unknown>);
				}
				throw new Error(`No executor for tool: ${def.name}`);
			}
		});
	}
