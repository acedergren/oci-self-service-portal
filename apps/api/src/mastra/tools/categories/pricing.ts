...
import {
	...
	AzurePricingClient,
	AWSPricingClient
} from '@portal/shared/pricing';
import type { WorkloadRequirements, CloudProvider } from '@portal/shared/pricing';
...
export const pricingTools: ToolEntry[] = [
	...
	},
	{
		...
		executeAsync: async (args) => {
			...
			return {
				error: 'Please specify a shapeName, architecture, or set listAll to true'
			};
		}
	},
	{
		...
		executeAsync: async (args) => {
			...
			if (args.skuName) {
				...
				if (!pricing)
					return {
						error: `SKU not found: ${args.skuName} in region ${region}`
					};
	...
	},
	{
		...
		executeAsync: async (args) => {
			...
			return {
				error: 'Please specify an instanceType, architecture, or set listAll to true'
			};
		}
	}
];
