...
export function getSystemPrompt(compartmentId?: string): string {
...
## NO PARALLEL TOOL CALLS WHEN ASKING QUESTIONS
...
// ── Static model allowlist (fallback when no providers configured in DB) ──

export const FALLBACK_MODEL_ALLOWLIST = [
...
export interface CloudAdvisorConfig {
	...
	memory?: Memory;
	/** Default OCI compartment ID for tool calls */
